...
function DrawControl({ areaType, onGeometryChange }) {
    ...
    useEffect(() => {
        ...
        const handleCreated = (e) => {
            featureGroup.clearLayers(); // only allow one shape at a time
...
function StreetSearch({ onLocationFound }) {
    ...
    const handleSelect = (result) => {
        onLocationFound([parseFloat(result.lat), parseFloat(result.lon)]);
        setQuery(result.display_name.split(",")[0]); // just the street name
    ...
    return (
        <div className="mb-3 relative">
            ...
            {showResults && results.length > 0 && (
                <ul className="absolute z-10 bg-white border border-line rounded-sm w-full mt-1 max-h-40 overflow-y-auto text-sm">
                    {results.map((r, i) => (
                        <li
                            key={i}
    ...
    export default function ApproveVolunteerModal({ volunteer, onClose, onApproved }) {
        ...
        const [flyToPosition, setFlyToPosition] = useState(null); // For map fly-to after search   
        ...
        return (
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
                <div className="bg-white rounded-md p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    ...
                                    setDrawnGeoJson(null); // reset since drawing tool changes
                                }}
                            >
                    ...
                    <StreetSearch onLocationFound={setFlyToPosition} />
